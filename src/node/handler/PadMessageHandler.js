...
exports.handleMessage = async function(client, message)
{
  ...
  if (env === 'production') {
    const clientAddress = (settings.trustProxy && client.handshake.headers['x-forwarded-for']) ? client.handshake.headers['x-forwarded-for'] : client.handshake.address;
    try {
      await rateLimiter.consume(clientAddress); // consume 1 point per event from IP
    }catch(e){
      console.warn("Rate limited: ", clientAddress, " to reduce the amount of rate limiting that happens edit the rateLimit values in settings.json");
